...
@section('content')
    <form method="POST" action="{{ $menu->exists ? route('admin.menus.update', $menu) : route('admin.menus.store') }}" enctype="multipart/form-data"
        ...
        <div class="rounded-3xl border border-cream-200 bg-white p-6 shadow-sm lg:self-start">
            ...
            <div x-data="{ preview: @js($menu->image ? asset($menu->image) : null) }">
                ...
                <p class="mt-2 text-xs text-navy-700">JPG, PNG, WEBP, atau SVG. Maks 2 MB. Kosongkan untuk mempertahankan foto saat ini.</p>
            </div>
        </div>
    </form>
@endsection